...
import { readableStreamToString, VisierPermissionManagementError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierPermissionManagementError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierPermissionManagementError(e, e.response?.data)
                }
                if (innerError instanceof VisierPermissionManagementError) {
                    // Got "VisierPermissionManagementError" from the above try block
                    throw innerError;
                }
